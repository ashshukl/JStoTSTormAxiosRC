...
class RoutingControllers {
    ...
    executeAction(actionMetadata, action, interceptorFns) {
        ...
        return Promise.all(paramsPromises)
            ...
            return this.handleCallMethodResult(result, actionMetadata, action, interceptorFns);q
        })
            .catch(error => {
    ...
    handleCallMethodResult(result, action, options, interceptorFns) {
        if ((0, isPromiseLike_1.isPromiseLike)(result)) {
            ...
            if (interceptorFns) {
                const awaitPromise = (0, runInSequence_1.runInSequence)(interceptorFns, interceptorFn => {
                    const interceptedResult = interceptorFn(options, result);
                    if ((0, isPromiseLike_1.isPromiseLike)(interceptedResult)) {
        ...
        return uses.map(use => {
            if (use.interceptor.prototype && use.interceptor.prototype.intercept) {
                ...
                return function (action, result) {
                    return (0, container_1.getFromContainer)(use.interceptor, action).intercept(action, result);
                };
            }
